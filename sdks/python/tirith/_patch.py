...
from tirith._config import get_proxy_url
...
def _patch_anthropic(proxy_url: str) -> None:
    ...
    if not getattr(anthropic.Anthropic.__init__, "_tirith_patched", False):
        anthropic.Anthropic.__init__ = _patched_init
        anthropic.Anthropic.__init__._tirith_patched = True
    ...
    if hasattr(anthropic, "AsyncAnthropic"):
        ...
        if not getattr(anthropic.AsyncAnthropic.__init__, "_tirith_patched", False):
            anthropic.AsyncAnthropic.__init__ = _patched_async_init
            anthropic.AsyncAnthropic.__init__._tirith_patched = True
...
def _patch_openai(proxy_url: str) -> None:
    ...
    if not getattr(openai.OpenAI.__init__, "_tirith_patched", False):
        openai.OpenAI.__init__ = _patched_init
        openai.OpenAI.__init__._tirith_patched = True
    ...
    if hasattr(openai, "AsyncOpenAI"):
        ...
        if not getattr(openai.AsyncOpenAI.__init__, "_tirith_patched", False):
            openai.AsyncOpenAI.__init__ = _patched_async_init
            openai.AsyncOpenAI.__init__._tirith_patched = True
